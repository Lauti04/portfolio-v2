import type { Dictionary } from './types'

export const es: Dictionary = {
  a11y: {
    skipToContent: 'Saltar al contenido',
  },
  theme: {
    switchToLight: 'Cambiar a tema claro',
    switchToDark: 'Cambiar a tema oscuro',
  },
  language: {
    label: 'Cambiar idioma',
  },
  nav: {
    label: 'Navegación de la página',
    about: 'Sobre mí',
    experience: 'Experiencia',
    projects: 'Proyectos',
    contact: 'Contacto',
  },
  hero: {
    eyebrow: 'Desarrollador Full-Stack · React · TypeScript',
    headline: 'Construyo productos web limpios y rápidos.',
    status: 'Abierto a nuevas oportunidades',
    subline:
      'Desarrollo con React, TypeScript y Node.js — cuidando el rendimiento, la accesibilidad y el detalle.',
    viewWork: 'Ver proyectos',
    viewCv: 'Ver CV',
    cvLabel: 'CV',
    cvPreview: 'Vista previa',
    cvDownload: 'Descargar',
  },
  techMarquee: {
    caption: 'Herramientas y tecnologías con las que trabajo',
  },
  profile: {
    heading: 'Resumen de perfil',
    about:
      'Desarrollador Full-Stack Junior con Técnico Superior en Desarrollo de Aplicaciones Web, actualmente construyendo funcionalidades en producción en VyA Projects. Cómodo en todo el stack — de interfaces en React a PHP, SQL y Strapi — con hábito de código limpio, revisión de código y entregas estructuradas mediante Jira.',
    roleLabel: 'Rol',
    roleValue: 'Desarrollador Full-Stack Junior — VyA Projects',
    educationLabel: 'Educación',
    educationValue:
      'Técnico Superior — Desarrollo de Aplicaciones Web (DAW), DigitechFP',
    certificationValue: 'AI Fundamentals — Google (Coursera)',
    languagesLabel: 'Idiomas',
    languagesValue: 'Español (nativo) · Inglés (B2, certificado Cambridge)',
    connectLabel: 'Contacto',
    contactModal: {
      heading: 'Enviar un mensaje',
      explain:
        'Esto abre tu cliente de correo con el mensaje ya escrito — no se envía nada desde esta página.',
      nameLabel: 'Nombre',
      namePlaceholder: 'Tu nombre',
      emailLabel: 'Tu email',
      emailPlaceholder: 'tu@ejemplo.com',
      messageLabel: 'Mensaje',
      messagePlaceholder: '¿Qué te gustaría decir?',
      send: 'Abrir cliente de correo',
      close: 'Cerrar',
      whatsappCta: 'O escríbeme por WhatsApp',
      subjectTemplate: 'Contacto desde el portfolio: {name}',
    },
  },
  experience: {
    heading: 'Experiencia',
    subheading: 'Dónde he aplicado este stack.',
    items: [
      {
        id: 'vya',
        role: 'Desarrollador Full-Stack Junior',
        company: 'VyA Projects',
        location: 'Remoto, España',
        dateRange: 'Dic 2025 – Jun 2026',
        description:
          'Desarrollé y mantuve funcionalidades en varias aplicaciones web en producción — interfaces, modo oscuro, componentes reutilizables, formularios — mediante Jira y flujos de revisión basados en Git.',
      },
      {
        id: 'universalTelecom',
        role: 'Becario de Desarrollo Web',
        company: 'Universal Telecom',
        location: 'Málaga, España',
        dateRange: 'Mar 2025 – Jun 2025',
        description:
          'Rediseñé la interfaz y el componente de calendario de un sitio corporativo, sumando funcionalidades dinámicas con AJAX/Fetch y generación automática de informes en PHP.',
      },
    ],
  },
  projects: {
    heading: 'Proyectos Seleccionados',
    subheading: 'Una selección de proyectos full-stack, interfaces y experiencias web.',
    viewCode: 'Ver código',
    items: [
      {
        id: 'eventflow',
        title: 'EventFlow',
        description:
          'Aplicación full-stack para gestionar eventos, tareas y recordatorios, con autenticación, calendario interactivo y distintas vistas de planificación.',
      },
      {
        id: 'dulceEncanto',
        title: 'Dulce Encanto',
        description:
          'Sitio web full-stack para una pastelería, con catálogo de productos y herramientas de administración, desarrollado con PHP, MySQL, Bootstrap y JavaScript.',
      },
      {
        id: 'happyPaws',
        title: 'Happy Paws',
        description:
          'Landing page frontend para servicios de mascotas, basada en una plantilla y adaptada con HTML, CSS y JavaScript. Incluye slider, galería, servicios, ofertas y contenido de adopción.',
      },
      {
        id: 'malagaSupercars',
        title: 'Malaga Supercars',
        description:
          'Proyecto académico full-stack de catálogo de vehículos, con filtros de búsqueda y persistencia en MySQL, desarrollado con HTML, CSS y PHP.',
      },
    ],
  },
  strengths: {
    heading: 'Lo que aporto',
    subheading: 'Algunas cosas que no se ven en una lista de tecnologías.',
    reviewedLabel: 'Revisado',
    items: [
      {
        title: 'Aprendizaje rápido',
        description:
          'Pasé de la formación profesional a lanzar funcionalidades en producción en meses — me adapto rápido a nuevos stacks, herramientas y bases de código.',
      },
      {
        title: 'Experiencia real en producción',
        description:
          'Interfaces, modo oscuro, componentes reutilizables y formularios en dos empresas — con tickets reales de Jira y revisión de código basada en Git, no proyectos personales sueltos.',
      },
      {
        title: 'Desarrollo asistido por IA con criterio',
        description:
          'Cómodo usando herramientas como Cursor, ChatGPT, Claude y Gemini para ir más rápido — pero siempre con el criterio para decidir qué mantener, cuestionar o reescribir.',
      },
    ],
  },
  contact: {
    heading: '¿Trabajamos juntos?',
    subheading: 'Abierto a puestos full-time y proyectos freelance.',
    whatsapp: 'WhatsApp',
    copyEmail: 'Copiar email',
    copyPhone: 'Copiar teléfono',
    copied: '¡Copiado!',
  },
  footer: {
    github: 'Perfil de GitHub',
    linkedin: 'Perfil de LinkedIn',
    whatsapp: 'WhatsApp',
  },
}
